...
2. `supabase/migrations/` を**Supabase側で手動実行**する(CIによる自動適用はない。既存プロジェクトの
   運用と同様の制約)。0003番のマイグレーションは新規注文通知トリガーで、実行前にファイル内の
   `v_webhook_url` / `v_webhook_secret` を実際の値に書き換える必要がある(ファイル内のコメントを参照。
   当初`alter database ... set app.settings.xxx`でDB設定として持たせる案だったが、Supabaseの
   ホスティング環境ではSQL Editorの実行ロールにその権限がなく使えなかったため、関数本体に直接
   埋め込む方式に変更している)。
